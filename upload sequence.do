...
* Convert versions to numeric
	gen ver_dlw_num = real(substr(trim(veralt_dlw), 2, .))
	gen ver_gld_num = real(substr(trim(veralt_gld), 2, .))
	gen verm_dlw_num = real(substr(trim(vermast_dlw), 2, .))
	gen verm_gld_num = real(substr(trim(vermast_gld), 2, .))
	replace ver_dlw_num = 0 if (ver_dlw_num > ver_gld_num) & (verm_gld_num > verm_dlw_num)
...
* tagged as NA in the versions. Need to create two rows for these too. 
	replace ver_dlw_num = 0 if veralt_dlw == "NA"
...
*keep if ver_gld_num > ver_dlw_num
	gen n_new = ver_gld_num - ver_dlw_num
...
* Expand to fill in intermediate versions for harmonized
	...
	gen new_vernum = ver_dlw_num + offset 
...
* Identify original to reclassify gap surveys as "2 - update harm"
	...
	gen ver_gld_num2 = real(substr(trim(veralt_gld), 2, .))
	gen verm_gld_num2 = real(substr(trim(vermast_gld), 2, .))

	replace case_type = "2 - update harm" if inrange(ver_gld_num2, 2, 99) & inrange(verm_gld_num2, 1, 99) & inlist(case_type, "3 - update raw & harm", "4 - new upload, several versions")
...
* Clean up
	drop offset new_vernum ver_dlw_num ver_gld_num n_new p1 surveyid_prefix base_path survey_folder surveyid1-surveyid8
...
* Loop over each folder
	forvalues i = 1/`=_N' {
		...
		* Run XML export only if the .dta file exists
		...
		if _rc == 0 {
			...
			file write tranxlog "`sid',NA,NA,Dataset is not available! Check" _n
			file close tranxlog
			restore
			continue
		}
	...
	* because shell robocopy will not accept forward slashes
		local gldfolder_bws = subinstr("${gldfolder}", "/", "\\", .)
		local primusfolder_bws = subinstr("${primusfolder}", "/", "\\", .)

		local folder_to_zip = "`gldfolder_bws'\\`country'\\`survey_prefix'\\`sid'"
		local zip_dest = "`primusfolder_bws'\\ZIP files\\`sid'.zip"

		local clean_folder = "`primusfolder_bws'\\temp\\primus_clean_`sid'"
	...
	* Build source folder and destination zip path
		local folder_to_zip_r = "`gldfolder_bws'\\`country'\\`survey_prefix'\\`sid_base'"
		local zip_dest_r = "`primusfolder_bws'\\ZIP files\\`sid_base'.zip"
	...
	* I will not include data sub-folder in the zip file bc presumably this is the file big in size
		...
		if `folder_size_num_r' < filelimit {
		    ...
		    tempname clean
			local clean_rawfolder = "`primusfolder_bws'/temp/primus_clean_`sid_base'"
